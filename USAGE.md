...
App manifests and listing images are generated by `scripts/build.mjs` from the definitions and checked-in source photographs; do not hand-edit `app.json`. The brand and driver SVGs are separate assets. Credentials, hardware IDs, or live account captures must not be committed. Full end-to-end behavior requires an online Toniebox 2 and a Homey Pro; mocked tests establish wire formats and app logic, not physical device effects.

## Releases

Use Conventional Commits for new changes: `fix:` and `perf:` select a patch release, `feat:` selects a minor release, and `!` or `BREAKING CHANGE:` selects a major release. Existing nonconventional history is preserved and included as earlier work; it is not rewritten. Documentation/maintenance-only releases require an explicit bump. Homey accepts stable `major.minor.patch` versions, not prerelease suffixes.

`npm run release -- --dry-run` previews the next version from commits after the latest reachable `vX.Y.Z` tag. `npm run release -- --bump auto` updates `package.json`, `package-lock.json`, `app.json`, `CHANGELOG.md`, and `.homeychangelog.json` together; explicit `patch`, `minor`, and `major` choices cannot understate the detected change level. Start from a clean checkout. The command does not commit, tag, push, or submit an app.

The manually dispatched **Prepare GitHub release** workflow performs the version update, runs store validation/tests/memory checks/audit, commits the generated files, and atomically pushes a version tag and release commit before creating GitHub release notes. It never force-pushes and never submits to the Homey App Store. Store submission remains a separate approval step after real-hub acceptance testing.
